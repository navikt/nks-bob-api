...
suspend fun main() {
    ...
    logger.info { "Deleting ignored words" }
    val response = httpClient.post("${config.api.url}/api/v1/admin/jobs/delete-ignored-words") {
        header(HttpHeaders.Authorization, "Bearer $token")
    }

    if (!response.status.isSuccess()) {
        val error = response.body<ErrorResponse>()
        logger.error { "Error from delete-ignored-words: ${error.code} ${error.message}" }
        throw IllegalStateException(
            "delete-ignored-words job failed with status ${response.status}: ${error.code} ${error.message}"
        )
    }

    val result = response.body<DeleteIgnoredWordsSummary>()
    logger.info { "Deleted ${result.deletedWords} words" }
}
