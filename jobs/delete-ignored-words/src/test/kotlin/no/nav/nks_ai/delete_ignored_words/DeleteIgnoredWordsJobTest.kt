...
/**
 * Tester for delete-ignored-words-jobben.
 *
 ...
 */
class DeleteIgnoredWordsJobTest {
    ...
    private fun stubApiSuccess(deletedWords: Int = 7) {
        wireMock.stubFor(
            post(urlEqualTo("/api/v1/admin/jobs/delete-ignored-words"))
                .willReturn(
                    aResponse()
                        .withStatus(200)
                        .withHeader("Content-Type", "application/json")
                        .withBody("""{"deletedWords":$deletedWords}""")
                )
        )
    }
    ...
    fun `main - kjorer uten feil ved vellykket API-respons`() = runBlocking {
        configureTest()
        stubApiSuccess(deletedWords = 7)

        main()
    }

    @Test
    fun `main - kjorer uten feil naar ingen ord slettes`() = runBlocking {
        configureTest()
        stubApiSuccess(deletedWords = 0)
    ...
    fun `main - sender riktig Authorization-header til API-et`() = runBlocking {
        ...
        wireMock.verify(
            postRequestedFor(urlEqualTo("/api/v1/admin/jobs/delete-ignored-words"))
    ...
    fun `main - kaller endepunktet noyaktig en gang`() = runBlocking {
        ...
        wireMock.verify(1, postRequestedFor(urlEqualTo("/api/v1/admin/jobs/delete-ignored-words")))
    }
    ...
    fun `main - kaller ikke API-et naar Texas-token feiler`() = runBlocking {
        ...
        wireMock.verify(0, postRequestedFor(urlEqualTo("/api/v1/admin/jobs/delete-ignored-words")))
    }
}
